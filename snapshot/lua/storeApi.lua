...
local function new_storeApi(host, basePath, schemes)
	local schemes_map = {}
...
function storeApi:placeOrder(body)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		path = string.format("%s",
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapi_Order.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function storeApi:getOrderById(orderId)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		path = string.format("%s",
			self.basePath, orderId);
	})
...
function storeApi:deleteOrder(orderId)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		path = string.format("%s",
