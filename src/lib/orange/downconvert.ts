...
var HTTP_METHODS = ['get', 'put', 'post', 'delete', 'options', 'head', 'patch', 'trace'],
    SCHEMA_PROPERTIES = ['format', 'minimum', 'maximum', 'exclusiveMinimum', 'exclusiveMaximum', 'minLength', 'maxLength', 'multipleOf', 'minItems', 'maxItems', 'uniqueItems', 'minProperties', 'maxProperties', 'x-additionalProperties', 'pattern', 'enum', 'default'],
...
Converter.prototype.convertSecurityDefinitions = function() {
    ...
    for (var secKey in this.spec.securityDefinitions) {
        ...
        if (security.type === 'http' && security.scheme === 'basic') {
            ...
            if (flow.authorizationUrl) security.authorizationUrl = flow.authorizationUrl;
            if (flow.tokenUrl) security.tokenUrl = flow.tokenUrl;
